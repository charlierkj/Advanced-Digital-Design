...
module bitcoin_hash (input logic clk, reset_n, start, 
  ...
  function logic [31:0] wtnew(input logic [3:0] n);//function with no inputs
    logic [31:0] s0, s1;
	 
	 s0 = rightrotate(w1[n], 7) ^ rightrotate(w2[n], 18) ^ (w1[n]>>3);
	 s1 = rightrotate(w14[n], 17) ^ rightrotate(w14[n], 19) ^ (w14[n]>>10);
	 wtnew = w0[n] + s0 + w9[n] + s1;
    ...
    if(!reset_n) begin
	 ...
	 case(state)
	   ...
	   PREP2: begin
		...
		PREP3: begin
		    for (int n = 0; n < num_nonces; n++) temp[n] <= w15[n] + k[t] + H[n];
			 w15[0] <= h2_0[index_h2];
			 w15[1] <= h2_1[index_h2];
			 w15[2] <= h2_2[index_h2];
			 w15[3] <= h2_3[index_h2];
		    ...
		    if (block == 0) begin 
			   ...
			   if (t<64) begin
                  ...
                  for (int n = 0; n < num_nonces; n++) w15[n] <= mem_read_data;
			       ...
			       for (int n = 0; n < num_nonces; n++) w15[n] <= wtnew(n);
		        end
				  for (int n = 0; n < num_nonces; n++) begin
				    temp[n] <= w15[n] + k[t+1] + G[n];
					 {A[n], B[n], C[n], D[n], E[n], F[n], G[n], H[n]} <= sha256_op(A[n], B[n], C[n], D[n], E[n], F[n], G[n], temp[n]);
